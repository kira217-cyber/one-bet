...
const SingleUserWeeklyBonusHistory = ({ userId }) => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const { data } = await api.get(
        `/api/weekly-bonus/admin/users/${userId}/claims`,
        {
          params: {
            page,
            limit: 15,
      ...
      toast.error(
        error?.response?.data?.message ||
          "Failed to load weekly bonus claim history",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-green-200/70">
            Single user weekly fixed bonus claim records
          </p>
        </div>
        ...
        <table className="w-full min-w-[850px] border-separate border-spacing-y-3">
          <thead>
            <tr className="text-left text-sm text-green-200/80">
              ...
              <th className="px-4 py-2">Period Days</th>
          ...
          <tbody>
            {claims.map((claim) => (
              ...
              >
                ...
                <td className="px-4 py-4">{claim.periodDays} Days</td>
                ...
                <td className="px-4 py-4">{formatDate(claim.claimedAt)}</td>
            ...
            {loading && (
              <tr>
                <td
                  ...
                  className="rounded-xl bg-green-950/20 px-4 py-10 text-center text-green-200/70"
                >
                  Loading weekly bonus claim history...
                </td>
              </tr>
            )}
          </tbody>
...
export default SingleUserWeeklyBonusHistory;
