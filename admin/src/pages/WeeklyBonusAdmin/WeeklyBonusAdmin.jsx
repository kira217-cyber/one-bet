...
const WeeklyBonusAdmin = () => {
  ...
  const loadSettings = async () => {
    try {
      ...
      const { data } = await api.get("/api/weekly-bonus/admin/settings");
  ...
  const loadClaims = async () => {
    try {
      ...
      const { data } = await api.get("/api/weekly-bonus/admin/claims", {
        params,
      });
  ...
  const handleChange = (field, value) => {
    if (field === "title.en" || field === "title.bn") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        title: form.title,
        periodDays: Number(form.periodDays || 7),
      ...
      if (editingId) {
        await api.put(`/api/weekly-bonus/admin/settings/${editingId}`, payload);
        toast.success("Weekly bonus updated");
      } else {
        await api.post("/api/weekly-bonus/admin/settings", payload);
        toast.success("Weekly bonus created");
      }
  ...
  const handleToggle = async (setting) => {
    try {
      await api.put(`/api/weekly-bonus/admin/settings/${setting._id}`, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/api/weekly-bonus/admin/settings/${id}`);
      toast.success("Weekly bonus deleted");
      loadSettings();
    } catch (error) {
        ...
        <div className="rounded-2xl border border-green-700/40 bg-gradient-to-r from-black/90 via-green-950/50 to-black/90 p-5 shadow-2xl shadow-green-950/30">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-black text-white">Weekly Bonus</h1>
                <p className="text-sm text-green-200/80">
                  Fixed period bonus for all users
                </p>
              </div>
            </div>
              ...
              <div className="mb-5 flex items-center justify-between gap-3">
                <div>
                  <h2 className="text-xl font-black text-white">
                    {editingId ? "Update Weekly Bonus" : "Add Weekly Bonus"}
                  </h2>
                  <p className="text-sm text-green-200/70">
                    Example: every 7 days user can claim ৳50
                  </p>
                </div>
            ...
            <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 className="flex items-center gap-2 text-xl font-black text-white">
                  ...
                  Weekly Bonus Claim History
                </h2>
                <p className="text-sm text-green-200/70">
                  All users fixed bonus claim records
                </p>
              </div>
              ...
              <div className="xl:col-span-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-green-700/50 bg-black/70 py-3 pl-11 pr-4 text-white outline-none placeholder:text-green-300/50 focus:border-green-400 focus:ring-2 focus:ring-green-400/30"
                  />
                </div>
              </div>
                  ...
                  {!loadingClaims && claims.length === 0 && (
                    <tr>
                      <td
                        ...
                        className="rounded-xl bg-green-950/20 px-4 py-10 text-center text-green-200/70"
                      >
                        No weekly bonus claim history found
                      </td>
                    </tr>
                  )}
...
const Input = ({ label, className = "", ...props }) => (
  <div className={className}>
    ...
    <input
      {...props}
      className="w-full rounded-xl border border-green-700/50 bg-black/70 px-4 py-3 text-white outline-none placeholder:text-green-300/50 focus:border-green-400 focus:ring-2 focus:ring-green-400/30"
    />
  </div>
);
...
const Select = ({ label, children, ...props }) => (
  <div>
    ...
    <select
      {...props}
      className="w-full cursor-pointer rounded-xl border border-green-700/50 bg-black/70 px-4 py-3 text-white outline-none focus:border-green-400 focus:ring-2 focus:ring-green-400/30"
    >
      {children}
    </select>
  </div>
...
export default WeeklyBonusAdmin;
