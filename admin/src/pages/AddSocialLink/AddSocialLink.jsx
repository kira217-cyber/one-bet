...
const AddSocialLink = () => {
  ...
  const fetchSocialLinks = async () => {
    try {
      setLoading(true);
      const res = await api.get("/api/admin/social-link");
  ...
  const sortedItems = useMemo(() => {
    return [...items].sort((a, b) => {
      ...
      return new Date(b?._id || 0) - new Date(a?._id || 0);
    });
  }, [items]);
  ...
  const resetForm = () => {
    ...
    const fileInput = document.getElementById("social-icon-input");
    ...
    try {
      ...
      if (editingId) {
        const res = await api.put(
          `/api/admin/social-link/${editingId}`,
          payload,
          {
            headers: {
        ...
        toast.success(res?.data?.message || "Social link updated successfully");
      } else {
        const res = await api.post("/api/admin/social-link", payload, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await api.delete(`/api/admin/social-link/${id}`);
      toast.success(res?.data?.message || "Social link deleted successfully");
  ...
  const handleQuickToggle = async (item) => {
    try {
      ...
      const res = await api.put(`/api/admin/social-link/${item._id}`, payload, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
        ...
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-green-200/80 md:text-base">
              Add, edit, delete and manage floating social icons.
            </p>
          </div>
          ...
          >
            <div className="border-b border-green-600/20 bg-gradient-to-r from-green-600/20 to-emerald-500/10 px-5 py-4">
              ...
              <p className="mt-1 text-sm text-green-200/80">
                Manage all social icons from here.
              </p>
            </div>
              ...
              {loading ? (
                ...
                <div className="flex min-h-[260px] flex-col items-center justify-center rounded-3xl border border-dashed border-green-500/20 bg-black/30 px-6 text-center">
                  ...
                  <p className="mt-2 max-w-md text-sm text-green-200/70">
                    Add your first social icon from the form.
                  </p>
                </div>
              ) : (
