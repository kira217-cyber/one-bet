...
const AddFeaturedGames = () => {
  ...
  const fetchFeaturedGames = async (isRefresh = false) => {
    try {
      if (isRefresh) setRefreshing(true);
      else setLoading(true);

      const { data } = await api.get("/api/admin/featured-games");
  ...
  const onEdit = (row) => {
    ...
    setPreview(
      row?.bannerImage
        ? `${import.meta.env.VITE_APP_URL}${row.bannerImage}`
        : "",
    );
    ...
    try {
      ...
      if (isEditMode) {
        const { data } = await api.put(
          `/api/admin/featured-games/${editingId}`,
          payload,
          {
        ...
        if (!data?.success) {
          throw new Error(data?.message || "Failed to update featured game");
        }

        toast.success("Featured game updated successfully");
      } else {
        const { data } = await api.post("/api/admin/featured-games", payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });

        if (!data?.success) {
          throw new Error(data?.message || "Failed to create featured game");
        }

        toast.success("Featured game created successfully");
      }

      resetForm();
      fetchFeaturedGames(true);
    } catch (error) {
      toast.error(
  ...
  const onDelete = async (id) => {
    const ok = window.confirm(
      "Are you sure you want to delete this featured game?",
    );
    if (!ok) return;

    try {
      const { data } = await api.delete(`/api/admin/featured-games/${id}`);

      if (!data?.success) {
        throw new Error(data?.message || "Failed to delete featured game");
      }

      toast.success("Featured game deleted successfully");
  ...
  const totalItems = useMemo(() => list.length, [list]);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-green-200/70">
                Create, update and delete featured game banners for your client
                site.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <button
                type="button"
                onClick={() => fetchFeaturedGames(true)}
        ...
        <div className={`${cardBase} p-5 sm:p-6`}>
          <div className="flex items-center gap-3 mb-5">
            <FaStar className="text-green-300 text-lg" />
            <h2 className="text-lg font-extrabold text-green-200">
              {isEditMode ? "Update Featured Game" : "Create Featured Game"}
            </h2>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            <div className="flex flex-col sm:flex-row gap-5 items-start">
              <div className="w-52 h-32 rounded-2xl border border-green-700/30 bg-black/40 flex items-center justify-center overflow-hidden">
                {preview ? (
                  <img
                    src={preview}
                    alt="preview"
              ...
              {list.map((row) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className="w-28 h-20 rounded-2xl border border-green-700/30 bg-black/40 overflow-hidden flex items-center justify-center shrink-0">
                      {row.bannerImage ? (
                        <img
                          src={`${import.meta.env.VITE_APP_URL}${row.bannerImage}`}
                          alt="featured-game"
                          className="w-full h-full object-cover"
                        />
                      ) : (
                        <FaStar className="text-3xl text-green-300/60" />
                      )}
                    </div>
...
export default AddFeaturedGames;
